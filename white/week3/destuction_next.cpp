...
int main(){
    for (int i : {0, 1}){
        ...
        cout << "Step "<< i << ": " << 2 << "\n";
    /*
    После каждой итерации объект route будет уничтодаться.
    На втором cout объект должен будет уничтожиться, поэтому
    в отладчике мы видим как мы заходим в диструктор
    через строчку 136 (Route route;)
    */
    }
    cout << "End\n";
    return 0;
