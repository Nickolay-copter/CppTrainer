...
int main(){
    GetRoute();
    /* 
    Объект как создастся так и уничтожится. Так как мы его
    никуда не сохранили и он больше не используется, его надо
    сразу уничтожить!
    */
    cout << 2 << "\n";
    return 0;
...
/*
- Объект уничтожается как только выходит из области
вилимости
- При уничтожении объекта вызывается деструктор
- Как правило достаточно деструктора сгенерированного
компиляторов
- С помощью описанного приёма можно исследовать, когда
